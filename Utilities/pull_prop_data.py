...
def pull_prop_data(espm_id, year_ending, month_ending, domain, auth):
    ...
    # Check if there is more than one year for comparison
    if annual_df.shape[0] > 1:
        ...
        for year in annual_df['Year Ending'].iloc[:-1]:
            ...
            try:                
                ...
                if final_value != 0:
                    eui_percent_changes.append((np.nan, year.strftime('%m/%d/%Y')))
                else:
                    eui_percent_changes.append((float('inf'), year.strftime('%m/%d/%Y')))
        ...
        best_wui_change_year = annual_df['Year Ending'].item().strftime('%m/%d/%Y')
        best_wui_change_value = 0
